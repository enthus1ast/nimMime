...
suite "attachements":
  ...
  test "string needs encoding":
    check "foo".needsEncoding() == false
    check "föö".needsEncoding() == true
  ...
  test "multipart test":
    ...
    check first.needsEncoding() == false
    ...
    check second.needsEncoding() == false
    ...
    check third.needsEncoding() == true
  ...
  test "concentrate 3 MimeMessages into 1":
    ...
    check $multi.header == """{"foo": @["in multi 1"]}"""
    check $multi.body == "in multi 1"

    var multi2 = newMimeMessage()
    multi2.header["foo"] = "in multi 2"
    multi2.body = "in multi 2"

    check $multi2.header == """{"foo": @["in multi 2"]}"""
    check $multi2.body == "in multi 2"
    ...
    check $multi.header == "{\"foo\": @[\"in multi 1\"]}"
    check $multi.body == "in multi 1"
    check ($multi.parts).mimeEncoder(QUOTED_PRINTABLES) == "=40=5Bfoo: in multi 2=0D=0A=0D=0Ain multi 2=0D=0A--=0D=0Afoo: in normal--4292486321577948087--=0D=0A=0D=0Ain normal=0D=0A----=0D=0A, foo: in normal--4292486321577948087--=0D=0A=0D=0Ain normal=5D"

  test "connection in headers":
    var test = newMimeHeaders()
    test["Connection"] = @["Upgrade", "Close"]
    check test["Connection", 0] == "Upgrade"
    check test["Connection", 1] == "Close"
    test.add("Connection", "Test")
    check test["Connection", 2] == "Test"
    check "upgrade" in test["Connection"]

  #[test "bug 5344":
    # Bug #5344. # TODO
    echo "start"
    echo $parseHeader("foobar: ")
    #check parseHeader("foobar: ") == ("foobar", @[""])
    echo "mid"
  ...
  test "mime headers and encoding":
    ...
    test.add("Connection", "Test")
    check $test == "{\"connection\": @[\"Test\"]}"

    msg.addHeaders(test)
    check msg.countLines() == 2
    check msg.mimeEncoder(QUOTED_PRINTABLES) == "connection: Test=0D=0A"

    msg.add(mimeNewline)
    check msg.countLines() == 3
    check msg.mimeEncoder(QUOTED_PRINTABLES) == "connection: Test=0D=0A=0D=0A"

    msg.add "body content"
    check msg.mimeEncoder(QUOTED_PRINTABLES)  == "connection: Test=0D=0A=0D=0Abody content"
    check msg.mimeEncoder(BASE64) == "Y29ubmVjdGlvbjogVGVzdA0KDQpib2R5IGNvbnRlbnQ="
